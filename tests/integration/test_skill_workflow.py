...
async def test_inventory_to_finding_and_next_candidate_across_resume(tmp_path):
    ...
    await tool.run({
        ...
        "artifact_ref": inventory.name, "current_phase": "analysis",
    }, None, allow)
    ids = {}
    ...
    (tmp_path / "xss-proof.txt").write_text(
    ...
    finder = ConfirmFindingTool(Store(str(tmp_path / "findings")), workflow=state)
    finding_args = {
    ...
    assert restored.completed_artifacts["web-enumeration"] == "inventory.md"
    ...
    await ConfirmFindingTool(
        Store(str(tmp_path / "findings")), workflow=restored,
