...
in a later test file once csrf is registered in INTENT_TO_SKILL /
...
SKILL_PATH = Path(__file__).parent.parent / "skills" / "csrf" / "SKILL.md"
...
def test_filename_pattern_is_sanitized_placeholder(skill):
    ...
    assert "findings/csrf-{sanitized-parameter-or-path}.md" in body
...
def test_never_writes_literal_placeholder_as_filename(skill):
    body = skill["body_flat"].lower()
