...
def format_iso(val: Any) -> Optional[str]:
    ...
    if isinstance(val, (int, float)):
        return datetime.fromtimestamp(val, timezone.utc).isoformat().replace(
            "+00:00", "Z"
        )
    return str(val)
...
async def main() -> int:
    ...
    try:
        ...
        sys.stderr.write(
            f"[kagent-browser-mcp] failed to start ingest server on :{args.port}: {err}\n"
        )
        return 1
    ...
    async def handle_call_tool(name: str, arguments: dict | None) -> types.CallToolResult:
        ...
        if name == "browser_capture_status":
                ...
                json.dumps(
                    {
                        ...
                        "lastActivityAt": format_iso(
                            get_val(s, "last_activity_at", "lastActivityAt")
                        ),
                    },
                    indent=2,
                )
            ...
            if not eps:
                return text_result(
                    "No endpoints captured yet. Confirm the extension is running with "
                    "capture enabled and the scope regex matches the target."
                )
                ...
                slim.append(
                    {
                        ...
                        "receivedAt": format_iso(
                            get_val(r, "received_at", "receivedAt")
                        ),
                    }
                )
